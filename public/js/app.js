...
export const response = (res, selector) => {
  ...
  res.msg.forEach((msg => {
    if (msg.msg !== undefined && msg.field !== undefined) {
      let selector = document.querySelector(`input[name="${msg.field}"]`);
      if(selector === null) {
        selector = document.querySelector(`textarea[name="${msg.field}"]`);
      }
      if(selector === null) {
        selector = document.querySelector(`select[name="${msg.field}"]`);
      }
     ...
     `);
        document.querySelector(`span[data-field="${msg.field}"]`).setAttribute('tooltip', msg.msg);
      }
    } else {
