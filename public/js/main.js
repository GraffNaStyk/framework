$.on('click', '.action', (e) => {
    ...
    $.post({url: e.target.dataset.url, data: data,}).then(res => res.json()).then(res => {
        $.loaderStop();
        if (res.isError) {
...
const Render = (e) => {
    $.loaderStart();
    $.render({url: e.target.dataset.url, el: e.target.dataset.el}).then(res => {
        $.loaderStop();
        if (res.isError) {
            message(res, e.target.dataset.url);
            return;
...
const message = (res, selector) => {
        ...
        lastRandom = rand;
        $.adjacent(form, ` <div data-${rand}="" class="alert alert-${res.ok ? 'success' : 'danger'} ${isModal ? 'mt-3' : ''}" role="alert"> ${res.msg} </div> `, isModal ? 'beforeend' : 'afterbegin');
    }
            ...
            if (contentType && contentType.indexOf('application/json') !== -1) {
                res.json().then(res => {
                    ...
                    if (res.params.modal !== undefined) {
                        Render({target: {dataset: {url: res.params.modal, el: 'modal'}}});
                        return;
...
;const throwFormErrors = (res, action) => {
    $.elements('span.err').forEach(e => e.remove());
    $.elements('.input_error').forEach(e => e.classList.remove('input_error'));
    $.elements('.switch__row').forEach(e => e.setAttribute('style', ''));
    ...
    if (Array.isArray(res.inputs)) {
        res.inputs.forEach((error => {
            ...
            if (selector !== null) {
                ...
                $.adjacent(row, ` <span class="err switch_error" data-field="${error.field}"> ${error.msg} </span>`);
                ...
                if (selector.parentElement.children[0].classList.contains('err') === false) {
                    $.adjacent(selector.parentElement, ` <span class="err" data-field="${error.field}"> ${error.msg} </span> `);
                }
            }
        }))
    }
